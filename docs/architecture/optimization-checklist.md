...
| RES-005 | `DELIVERED`（R5） | 审计声明、机器门禁、CI 与远端交付状态需要重新校准 | 当前文档、生成 fixture 与规则一致；锁定全量 `7684 passed, 9 skipped`，Application/Domain 覆盖率为 `81.77%` / `81.23%`，Pylint `10.00/10`、架构/兼容、真实启动和最终 exact-head GitHub CI 闭环，远端 `0/0` |
...
| 覆盖率低水位 | Application 81.77%，Domain 81.23% | Chain、Runtime、Agent、Adapter、Startup 未进入包级覆盖率门禁 |
